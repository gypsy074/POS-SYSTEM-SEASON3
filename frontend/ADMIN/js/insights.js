...
function renderAiReportCard() {
    const text = aiLastReport
        ? `<div class="ai-report-text"><p class="ai-report-source">
               <i class="fas ${aiLastReport.source === "ai" ? "fa-wand-magic-sparkles" : "fa-calculator"}"></i>
               ${aiLastReport.source === "ai" ? "Generated by AI" : "Statistics summary"} · ${aiLastReport.at}</p>
           <p>${aiLastReport.text.replace(/</g, "&lt;")}</p></div>`
        : "";
    return `
        <div class="ai-card">
...
async function generateAiReport() {
    ...
    const body = document.getElementById("aiHeaderPanelBody");
    if (body && window.__aiInsightsData) renderInsights(window.__aiInsightsData);
}
